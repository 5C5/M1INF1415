...
\end{verbatim}
Étape 1 :Remplacer ttes les tables dans  \varnothing(Tables(V)) par  \varnothing(V)\\
Étape 2 : Remplacer chaque colonne A $\in$ Groupes(Q) OU ColSel(Q) OU AggSel(Q) pour  \varnothing(B) avec B satisfaisant les conditions C2 et C3\\
Étape 3 : Déterminer une combinaison de prédicats prédéfinis CONDS' satisfaisant la condition C4. Remplace CONDS(Q) par CONDS'\\
Étape 4 : Considérons un aggrégat COUNT(A) $\in$ Sel(Q) tel que A $\in$ $ \varnothing$(Cols(V)) et A $\notin$(Sel(V)). Remplacer COUNT(A) par COUNT(B) avec B$\in \varnothing$ (V) pour B quelconque.\\
